...
脚本会定期采样 GPU 功耗并将结果保存到 outputs/power_measurements.csv
"""
import time
import argparse
import csv
...
def monitor_pid(pid, interval=0.1, device_index=0, out_csv='outputs/power_measurements.csv'):
    if pynvml is None:
...
def monitor_command(cmd, interval=0.1, device_index=0, out_csv='outputs/power_measurements.csv'):
    # start subprocess
...
def main():
    ...
    parser.add_argument('--out', type=str, default='outputs/power_measurements.csv', help='输出 CSV 路径')
