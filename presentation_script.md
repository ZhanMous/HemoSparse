HemoSparse — 复试演讲稿（中英双语，约 2 分钟）

【中文部分】

引言
大家好，我叫（你的名字）。今天我汇报的工作题目是《HemoSparse：面向隐私保护与边缘高效推理的稀疏脉冲医学影像分类研究》。本工作关注的问题是：在医学影像分类任务上，稀疏脉冲神经网络（SNN）是否可以在保证准确率的同时带来更好的能效与隐私鲁棒性。
...
Introduction
Hello, my name is (your name). The project I present is "HemoSparse: Sparse Spiking Neural Networks for Privacy-Aware and Edge-Efficient Medical Image Classification." The core question is whether sparse SNNs can achieve comparable accuracy while improving energy efficiency and offering different privacy robustness characteristics on medical image tasks.
...
---
文件已保存: [HemoSparse/presentation_script.md](HemoSparse/presentation_script.md)
